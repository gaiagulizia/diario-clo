 ...
 * RichTextSurface
 * ---------------------------------------------------------------------
 * L'editor di testo formattato vero e proprio: barra degli strumenti,
 * titolo, area di scrittura, e tutta la logica di luoghi, sottoschede,
 * evidenziatore, citazioni, elenchi da spuntare e annulla/ripristina.
 *
 * Usato identico sia per i fogli del diario (PageEditor) sia per le
 * sottoschede (SubcardEditor): l'unica differenza tra le due è cosa
 * viene passato come `topRow`/`bottomRow` (data, azioni, navigazione a
 * libro per i fogli; pulsante "torna alla pagina" per le sottoschede) e
 * come viene salvato il contenuto (onChange).
 * ---------------------------------------------------------------------
 */
export default function RichTextSurface({
  ...
  useEffect(() => {
    ...
    historyRef.current = { stack: [contentHtml || ''], index: 0 };
  ...
  function handleEditableClick(e) {
    ...
    if (subcardEl) {
      e.preventDefault();
      onNavigateToSubcard(subcardEl.dataset.subcardId);
    }
  }
    ...
    <div className="editor-wrap">
      ...
      <div className="page-sheet">
        ...
        <input
          ...
          placeholder={titlePlaceholder || 'Titolo'}
        />
